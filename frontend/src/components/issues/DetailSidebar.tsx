...
import { Issue, IssueSubCategory, UserRole, User as UserType } from '../../../types';
import { CATEGORY_COLORS } from '../../constants';
import {
  Send, MessageSquare, Clock, ShieldAlert, ChevronUp,
  User, Building2, Droplets, Zap, MoreHorizontal,
  Trash2, CheckCircle, RotateCcw, Play
} from 'lucide-react';
    ...
    <div className="h-full flex flex-col pb-24">
      ...
      <div className="relative h-32 bg-slate-100/50 dark:bg-slate-800/50 backdrop-blur-sm flex-shrink-0 rounded-t-2xl overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `linear-gradient(135deg, ${CATEGORY_COLORS[issue.category]} 0%, #ffffff 100%)` }}
        />
        ...
        <div className="mb-6">
          ...
          {issue.objectName && (
            <div className="flex flex-col gap-2 mb-4">
              <div className="flex items-center gap-2 bg-indigo-50/80 dark:bg-indigo-900/30 backdrop-blur-sm px-3 py-2 rounded-lg border border-indigo-200/50 dark:border-indigo-900/40">
                ...
                <span className="text-sm font-bold text-indigo-800 dark:text-indigo-200">{issue.objectName}</span>
              </div>
              {issue.subCategory && (
          ...
          <div className="flex gap-2 mb-4">
            <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide ${
              issue.status === 'Open' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' :
              issue.status === 'In Progress' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' :
              'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
            }`}>
              {issue.status === 'Open' ? 'Открыто' : issue.status === 'In Progress' ? 'В работе' : 'Решено'}
            </span>
            <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide ${
              ...
              issue.severity === 'Medium' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' :
              'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
            }`}>
              {issue.severity}
            </span>
          ...
          {issue.aiSummary && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900/30 rounded-xl p-3 mb-4">
              <p className="text-xs font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider mb-1">AI-анализ</p>
              <p className="text-sm text-blue-800 dark:text-blue-200">{issue.aiSummary}</p>
            </div>
          )}
        ...
        <div>
          ...
          <div className="space-y-3 mb-4">
            {issue.comments?.length === 0 && (
              <p className="text-sm text-slate-400 dark:text-slate-600 text-center py-4">Пока нет комментариев</p>
            )}
            {issue.comments?.map((comment, i) => (
              ...
              >
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 ml-8">{comment.text}</p>
              </div>
            ))}
          </div>
