        ...
        <div className="sticky top-0 z-10 bg-white/95 dark:bg-slate-950/95 backdrop-blur-sm border-b border-slate-100 dark:border-slate-800 px-6 py-4">
          ...
          {initialScores && (
            <div className="mt-3 flex items-center gap-4">
              <div
                className={`w-14 h-14 rounded-xl flex items-center justify-center font-black text-xl text-white flex-shrink-0 ${
                  initialScores.composite >= 60
                    ? "bg-emerald-500"
                    : initialScores.composite >= 30
                    ? "bg-amber-500"
                    : "bg-red-500"
                }`}
              >
          ...
          {loading ? (
            ...
            <>
              ...
              <section>
                ...
                {detail.objects?.byType &&
                Object.keys(detail.objects.byType).length > 0 ? (
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {Object.entries(
                      detail.objects.byType as Record<string, number>
                    ).map(([type, count]) => (
                      <StatCard
