...
**Каталог границ монтируется.** `backend/src/data/` подключён томом
(`./backend/src/data/:/app/src/data:rw,Z`). Без этого `/app/src/data` - содержимое
образа: `fetch-osm-boundaries.js` пишет файл внутрь работающего контейнера, любая
пересборка контейнер пересоздаёт, и импортёру читать нечего. Соседние JSON при
этом на месте, потому что приехали в образе, отчего выглядит так, будто скачивание
не сработало.

**Права на каталог.** Контейнер работает под непривилегированным `nodeapp`
(`USER nodeapp` в `backend/Dockerfile`), а смонтированный каталог принадлежит
тому, кто делал checkout. Один раз на сервере:

```
docker compose exec backend id
chown -R <UID>:<GID> backend/src/data
```

Загрузчики границ проверяют запись **до** обращения к Overpass и падают сразу с
подсказкой, а не после двух минут скачивания. Если права выдать нельзя, есть
`--out=` и можно писать в другой каталог.

Флаг монтирования `:z`, строчный. Заглавный `:Z` переметил бы каталог в приватную
для контейнера SELinux-метку, и доступ к нему потерял бы git checkout.

Скачанные `osm-*.geojson` **нужно коммитить**. GitLab Runner по умолчанию делает
`git clean -ffdx` перед сборкой, а каталог теперь смонтирован в контейнер, поэтому
неотслеживаемый файл удаляется на каждом пайплайне. В `.gitignore` для них стоит
исключение.
