// frontend/src/components/admin/AdminPromiseModal.tsx

import React, { useState, useEffect } from 'react';
import { X, CheckCircle2, Loader2, ClipboardList } from 'lucide-react';
import { CivicPromise, PromiseStatus } from '../../../types';

interface AdminPromiseModalProps {
  isOpen: boolean;
  editing: CivicPromise | null;
  allocationId: string | null;
  onClose: () => void;
  onSave: (data: Partial<CivicPromise>) => Promise<void>;
}

const STATUSES: PromiseStatus[] = ['Planned', 'In Progress', 'Pending Verification', 'Completed', 'Failed'];
...
export const AdminPromiseModal: React.FC<AdminPromiseModalProps> = ({
  isOpen, editing, allocationId, onClose, onSave
}) => {
  const [title,       setTitle]       = useState('');
  const [description, setDescription] = useState('');
  const [deadline,    setDeadline]    = useState('');
  const [status,      setStatus]      = useState<PromiseStatus>('Planned');
  ...
  useEffect(() => {
    if (editing) {
      ...
      setDeadline(editing.deadline ? new Date(editing.deadline).toISOString().split('T')[0] : '');
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden relative z-10 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
        <div className="p-5 bg-gradient-to-br from-emerald-600 to-teal-700 text-white relative">
          ...
          <h2 className="text-lg font-black tracking-tight">
            {editing ? 'Изменить обещание' : 'Новое обещание'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Детали, подробности..."
              rows={2}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Срок</label>
              <input
                type="date"
                value={deadline}
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Статус</label>
              <select
                value={status}
