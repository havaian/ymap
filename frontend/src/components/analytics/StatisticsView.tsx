...
function TrendChart({
  ...
  data: Array<{ label: string; total: number; resolved: number }>;
}) {
            ...
            <div className="w-full flex flex-col items-center justify-end h-32">
              ...
              >
                ...
                <div
                  ...
                  style={{
                    height: `${
                      d.total > 0 ? (d.resolved / d.total) * 100 : 0
                    }%`,
                  }}
                />
              </div>
...
function LoadingPulse({ className = "h-24" }: { className?: string }) {
  return (
    <div
      className={`bg-slate-100 dark:bg-slate-800 rounded-2xl animate-pulse ${className}`}
    />
  );
}
  ...
  const stats = useMemo(() => {
    ...
    issues.forEach((i) => {
      ...
      if (i.organizationId) {
        ...
        else if (org?.type === IssueCategory.HEALTH)
          byOrgType["Здравоохранение"]++;
      } else {
        byOrgType["Прочие (уличные)"]++;
      }
    });

    const categoriesSorted = Object.entries(byCategory).sort(
      (a, b) => b[1] - a[1]
    );
    const districtsSorted = Object.entries(byDistrict).sort(
      (a, b) => b[1] - a[1]
    );
      ...
      <div className="p-6 md:p-10 max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-2">
              Анализ состояния городской среды Ташкента на основе {stats.total}{" "}
              обращений граждан
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <StatCard
            ...
            color="red"
            percentage={(stats.open / stats.total) * 100}
          />
          <StatCard
            title="В процессе"
          ...
          <StatCard
            ...
            color="green"
            percentage={(stats.resolved / stats.total) * 100}
          />
          <StatCard
            title="KPI Решений"
          ...
          <StatCard
            ...
            sub={
              resolution?.overall.medianDays != null
                ? `Медиана: ${resolution.overall.medianDays} дн`
                : undefined
            }
          />
        </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">
                      Динамика обращений (12 мес)
                    </h3>
                  </div>
                  {trendsLoading || !trends ? (
                    <LoadingPulse className="h-36" />
                  ) : (
                    <>
                        ...
                        <div className="h-2.5 w-full bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor:
                                CATEGORY_COLORS[cat as IssueCategory] ||
                                "#64748b",
                            }}
                          />
                        </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="space-y-3">
                      {trends.categories.map((c) => (
                        <div
                          key={c.category}
                          className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl"
                        >
                          <div
                            className="w-2 h-10 rounded-full shrink-0"
                            style={{
                              backgroundColor:
                                CATEGORY_COLORS[c.category as IssueCategory] ||
                                "#64748b",
                            }}
                          />
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate-400">
                              {c.total} всего · {c.resolved} решено
                            </p>
                          </div>
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-sm text-blue-100 mb-6 leading-relaxed">
                        {resolution?.overall.avgDays != null
                          ? `Среднее время решения проблемы — ${
                              resolution.overall.avgDays
                            } дн. Медиана — ${
                              resolution.overall.medianDays ?? "—"
                            } дн.`
                      ...
                      <div className="flex items-center gap-3 bg-white/10 p-3 rounded-2xl backdrop-blur-md border border-white/10">
                        ...
                        <span className="text-lg font-black">
                          {resolution?.overall.count ?? stats.resolved}
                        </span>
                      </div>
                    </div>
                      ...
                      <div key={dist} className="flex flex-col gap-1.5">
                        ...
                        <div className="h-1.5 w-full bg-slate-50 dark:bg-slate-800 rounded-full">
                          <div
                            ...
                            style={{
                              width: `${(count / maxDistrictCount) * 100}%`,
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <div className="flex flex-wrap gap-4 justify-center">
                    {Object.entries(stats.bySeverity).map(([sev, count]) => (
                      <div key={sev} className="flex flex-col items-center">
                        <div
                          className={`w-14 h-14 rounded-full border-2 flex items-center justify-center font-black text-sm mb-2
                               ${
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">
                  За последний месяц зафиксировано{" "}
                  <span className="text-indigo-600 font-bold">
                    {stats.totalVotes}
                  </span>{" "}
                  взаимодействий с платформой. Ваша активность напрямую влияет
                  на приоритет исполнения работ государственными службами.
                </p>
              </div>
            ...
            {effLoading || !efficiency ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard
                    title="Общий бюджет"
                    value={fmtUzs(efficiency.summary.totalBudget)}
                    icon={<DollarSign className="text-indigo-500" />}
                    color="indigo"
                  />
                  <StatCard
                    title="Исполнение"
                    value={`${efficiency.summary.avgExecutionRate}%`}
                    icon={<Target className="text-green-500" />}
                    color="green"
                    percentage={efficiency.summary.avgExecutionRate}
                  />
                  <StatCard
                    title="Стоимость решения"
                    value={
                      efficiency.summary.avgCostPerResolved
                        ? fmtUzs(efficiency.summary.avgCostPerResolved)
                        : "—"
                    }
                    icon={<BarChart3 className="text-blue-500" />}
                    color="blue"
                  ...
                  <StatCard
                    title="Аномалии"
                    value={efficiency.summary.anomalyCount}
                    icon={<AlertTriangle className="text-red-500" />}
                    color="red"
                    sub="высокий бюджет, мало решений"
                  />
                </div>
                      ...
                      <tbody>
                        {efficiency.districts
                          ...
                          .map((d) => (
                            ...
                            >
                              <td className="py-3 pl-2 pr-4 font-bold text-slate-700 dark:text-slate-300">
                                {d.district}
                              </td>
                ...
                {efficiency.anomalies.length > 0 && (
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">
                        Аномалии ({efficiency.anomalies.length})
                      </h3>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 ml-12">
                      Учреждения с высоким исполнением бюджета, но низким %
                      решённых проблем
                    </p>
                    <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100">
                    Время решения по районам
                  </h3>
                </div>
                {resLoading || !resolution ? (
                  <LoadingPulse />
                ) : (
                  <div className="space-y-5">
                    {resolution.byDistrict.map((d) => {
                      const maxDays = Math.max(
                        ...resolution.byDistrict.map((x) => x.avgDays || 0),
                        1
                      );
                      return (
                        <div key={d.district} className="flex flex-col gap-1.5">
                          <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400">
                            ...
                            <span className="normal-case">
                              ...
                              <span className="font-normal text-slate-300 dark:text-slate-600 ml-1">
                                ({d.count})
                              </span>
                            </span>
                          </div>
              ...
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-6">
                  {resolution.byCategory.map((c) => {
                    const maxDays = Math.max(
                      ...resolution.byCategory.map((x) => x.avgDays || 0),
                      1
                    );
                    return (
                      <div key={c.category} className="space-y-2">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-tight">
                          ...
                          <span className="text-slate-800 dark:text-slate-100">
                            ...
                            <span className="font-normal text-slate-400 text-[10px] ml-1.5">
                              мин {c.minDays ?? "—"} / макс {c.maxDays ?? "—"}
                            </span>
                          </span>
                        </div>
                        <div className="h-2.5 w-full bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {efficiency.districts
                    ...
                    .map((d, i) => (
                      <div
                        key={d.district}
                        className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl flex items-center gap-3"
                      >
                        <span className="text-lg font-black text-slate-300 dark:text-slate-600 w-8 shrink-0">
                          #{i + 1}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">
                          ...
                          <p className="text-[10px] text-slate-400">
                            {d.orgCount} объектов · {d.totalIssues} обращений
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-xs font-black text-slate-700 dark:text-slate-300">
                            {fmtUzs(d.totalBudget)}
                          </p>
                          <p
