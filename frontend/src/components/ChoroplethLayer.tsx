...
export const ChoroplethLayer: React.FC<ChoroplethLayerProps> = ({
    ...
    useEffect(() => {
        ...
        analyticsAPI.getChoropleth(params)
            .then(res => {
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.bindTooltip(
                    `<div style="font-family: system-ui; font-size: 12px;">
                        ...
                        <span style="font-size: 18px; font-weight: 900; color: ${getColor(score)}">${score}</span>
            ...
            <div className="space-y-1">
                {grades.map((g, i) => (
                    <div key={g} className="flex items-center gap-2">
                        <div className="w-4 h-3 rounded-sm" style={{ backgroundColor: getColor(g) }} />
                        <span className="text-[10px] font-bold text-slate-500">
                            {i === 0 ? `${g}+` : i === grades.length - 1 ? `0–${grades[i - 1]}` : `${g}–${grades[i - 1]}`}
                        </span>
                    </div>
                ))}
            </div>
