    ...
    <div className="h-full flex flex-col pb-24">
      ...
      <div className="flex-1 overflow-y-auto pt-8 px-6 pb-6 custom-scrollbar">
        ...
        <div className="mb-6">
            ...
            <div className="flex gap-2 mb-4">
                 <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide
                    ${issue.status === 'Open' ? 'bg-red-50/80 dark:bg-red-900/30 backdrop-blur-sm text-red-600 dark:text-red-400 border border-red-200/50 dark:border-red-900/40' : 
                      issue.status === 'In Progress' ? 'bg-blue-50/80 dark:bg-blue-900/30 backdrop-blur-sm text-blue-600 dark:text-blue-400 border border-blue-200/50 dark:border-blue-900/40' : 'bg-green-50/80 dark:bg-green-900/30 backdrop-blur-sm text-green-600 dark:text-green-400 border border-green-200/50 dark:border-green-900/40'}
                `}>
                    {issue.status}
                </span>
                <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide
                    ${issue.severity === 'Critical' ? 'bg-rose-50/80 dark:bg-rose-900/30 backdrop-blur-sm text-rose-700 dark:text-rose-300 border border-rose-200/50 dark:border-rose-900/40' : 'bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur-sm text-slate-600 dark:text-slate-400 border border-slate-200/50 dark:border-slate-700/50'}
                `}>
            ...
            {issue.aiSummary && (
                <div className="bg-gradient-to-r from-violet-50/80 to-indigo-50/80 dark:from-violet-900/30 dark:to-indigo-900/30 backdrop-blur-sm border border-violet-200/50 dark:border-violet-900/40 rounded-xl p-4 flex gap-3">
        ...
        <div className="flex gap-4 mb-8">
            <button onClick={() => onUpvote(issue.id)} className="flex-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 hover:border-blue-300 dark:hover:border-blue-500 hover:bg-blue-50/80 dark:hover:bg-blue-900/20 transition rounded-xl p-3 flex flex-col items-center group cursor-pointer shadow-sm">
            ...
            <div className="flex-1 bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 rounded-xl p-3 flex flex-col items-center justify-center">
                ...
                {issue.comments.length === 0 ? (
                    ...
                    issue.comments.map(comment => (
                        <div key={comment.id} className="flex gap-3">
                            <div className="w-8 h-8 rounded-full bg-slate-200/80 dark:bg-slate-700/80 backdrop-blur-sm flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-slate-500 dark:text-slate-400" /></div>
                            <div className="flex-1">
                                <div className="bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-2xl rounded-tl-none">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-t border-slate-200/50 dark:border-slate-800/50 p-4">
        <form onSubmit={handleCommentSubmit} className="relative flex items-center gap-2">
            <input 
                type="text" placeholder="Write an update..." 
                className="flex-1 pl-4 pr-4 py-3 bg-slate-50/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 rounded-xl text-sm outline-none transition dark:text-white"
