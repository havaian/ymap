...
import {
  FacilityObject, Issue, Severity, User, UserRole, Task, BudgetAllocation
} from '../../../types';
import { AllocationSection } from '../promises/AllocationSection';
import { tasksAPI } from '../../services/api';
import {
  Building2, MapPin, CheckCircle2, Star, ChevronRight, Plus,
  Info, Calendar, Wifi, Zap, Droplets, Hammer, Users,
  Camera, X, ThumbsUp, ThumbsDown, Loader2, ChevronDown,
  ChevronUp, ImageIcon, ClipboardList, ClipboardCheck
} from 'lucide-react';
...
const TASK_STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  'Planned':              { label: 'Запланировано',  color: 'text-slate-500 bg-slate-100 dark:bg-slate-800' },
  'In Progress':          { label: 'Выполняется',    color: 'text-blue-600 bg-blue-50 dark:bg-blue-900/30' },
  'Pending Verification': { label: 'На проверке',    color: 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' },
  'Completed':            { label: 'Выполнено',      color: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30' },
  'Failed':               { label: 'Не выполнено',   color: 'text-rose-600 bg-rose-50 dark:bg-rose-900/30' }
};

// ── InfoField ─────────────────────────────────────────────────────────────────

function InfoField({ key, icon: Icon, label, value }: {
  key: string,
...
function VerifyForm({ taskId, onDone }: { taskId: string; onDone: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await tasksAPI.verify(taskId, { status, comment: comment.trim() || undefined, photoUrl });
      onDone();
    } catch (err: any) {
    ...
    <div className="mt-3 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-black transition-colors ${status === 'done' ? 'bg-emerald-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}`}
        >
          <CheckCircle2 size={15} /> Выполнено
        </button>
        <button
          onClick={() => setStatus('problem')}
          className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-black transition-colors ${status === 'problem' ? 'bg-red-500 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}`}
        >
          <X size={15} /> Проблема
        </button>
      </div>
      ...
      {photoPreview ? (
        <div className="relative w-full h-32 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
          ...
          <button onClick={() => { setPhoto(null); setPhotoPreview(null); }} className="absolute top-2 right-2 w-6 h-6 bg-black/60 rounded-full flex items-center justify-center text-white hover:bg-black/80">
            <X size={12} />
          </button>
        </div>
      ) : (
...
function TaskCard({ key, task, currentUser, onRefresh }: {
  key: string,
  task: Task;
  ...
  const barColor = total === 0 ? 'bg-slate-300 dark:bg-slate-600'
    ...
    <div className="rounded-2xl border border-slate-200/60 dark:border-slate-700/60 overflow-hidden bg-white/60 dark:bg-slate-900/40">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${cfg.color}`}>{cfg.label}</span>
            {task.deadline && (
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t border-slate-100 dark:border-slate-800">
          {task.description && (
            <p className="text-xs text-slate-600 dark:text-slate-400 pt-3">{task.description}</p>
          )}
          ...
          {total > 0 && (
            <div className="space-y-1">
              <div className="flex justify-between text-[10px] font-bold text-slate-500">
                ...
                {problem > 0 && <span className="text-red-500">{problem} ✗</span>}
              </div>
              <div className="h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barColor}`} style={{ width: `${Math.round((done / total) * 100)}%` }} />
              </div>
            </div>
          )}
          ...
          {task.verifications?.slice(0, 3).map(v => (
            <div key={v._id} className="flex items-start gap-2 text-xs">
              <span className={`font-black flex-shrink-0 ${v.status === 'done' ? 'text-emerald-500' : 'text-red-500'}`}>
                {v.status === 'done' ? '✓' : '✗'}
              </span>
              <div className="flex-1 min-w-0">
                ...
                {v.photoUrl && (
                  <a href={`/api/uploads/${v.photoUrl}`} target="_blank" rel="noopener noreferrer"
          ...
          {currentUser && (
            <div className="pt-1">
              {verifying ? (
                <VerifyForm taskId={task.id} onDone={() => { setVerifying(false); onRefresh(); }} />
              ) : (
                <button
...
function TasksSection({ objectId, currentUser }: { objectId: string; currentUser: User | null }) {
  ...
  if (tasks.length === 0) return (
    <p className="text-xs text-slate-400 dark:text-slate-600 text-center py-4">Нет задач</p>
  );

  return (
    <div className="space-y-2">
      {tasks.map(t => (
        <TaskCard key={t.id} task={t} currentUser={currentUser} onRefresh={load} />
      ))}
    </div>
  );
}
...
export const ObjectSidebar: React.FC<ObjectSidebarProps> = ({
  object, issues, currentUser, onClose, onIssueClick, onReportIssue
}) => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-6 pt-8 pb-6" style={{ background: `linear-gradient(135deg, ${typeColor}15, ${typeColor}05)` }}>
        <div className="flex items-start gap-4">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg" style={{ backgroundColor: typeColor }}>
            <Building2 size={24} className="text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider text-white mb-2" style={{ backgroundColor: typeColor }}>
              {typeLabel}
            </div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white leading-tight">{object.name}</h2>
            {object.nameRu && object.nameRu !== object.name && (
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{object.nameRu}</p>
            )}
            {(object.viloyat || object.tuman) && (
              <div className="flex items-center gap-1 mt-1.5 text-slate-500 dark:text-slate-400">
                <MapPin size={12} />
                <span className="text-xs font-bold">{[object.tuman, object.viloyat].filter(Boolean).join(', ')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {stats.total > 0 && (
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              { label: 'Всего', value: stats.total, color: 'text-slate-600 dark:text-slate-300' },
              { label: 'Активных', value: stats.active, color: 'text-red-500' },
              { label: 'Решено', value: stats.resolved, color: 'text-emerald-500' }
            ].map(s => (
              <div key={s.label} className="bg-white/60 dark:bg-slate-800/40 rounded-xl p-2 text-center">
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 pt-4 pb-1">
            ...
            <div className="bg-white/60 dark:bg-slate-800/30 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 px-4 py-1">
              {Object.entries(object.details).map(([key, val]) => {
                ...
                return (
                  <InfoField key={key} icon={Icon} label={CONDITION_LABELS[key] || key} value={String(val)} />
                );
              })}
            </div>
          </div>
        ...
        {isAdmin && (
          <div className="px-6 pt-4 pb-1">
            ...
            <AllocationSection targetType="object" targetId={object.id} currentUser={currentUser} />
          </div>
        )}
          ...
          <div className="space-y-2.5">
            {objectIssues.length > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`text-[9px] font-black uppercase ${
                        ...
                        : issue.status === 'In Progress' ? 'text-blue-500'
                        : 'text-red-500'
                      }`}>{issue.status}</span>
                    ...
                    {issue.votes > 0 && (
                      <p className="text-[10px] text-slate-400 mt-0.5">▲ {issue.votes} голосов</p>
                    )}
                  </div>
              ...
              <div className="py-8 text-center">
                ...
                <p className="text-sm font-bold text-slate-400 dark:text-slate-600">Обращений не зарегистрировано</p>
              </div>
            )}
          </div>
