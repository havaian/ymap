...
export const AllocationSection: React.FC<AllocationSectionProps> = ({
  ...
  }>({ open: false, editing: null, allocationId: null });
  ...
  const handleDeleteAllocation = async (id: string) => {
    if (
      !confirm(
        "Удалить выделение? Связанные задачи останутся, но будут откреплены."
      )
    )
      return;
    ...
    setTasks((prev) =>
      prev.map((t) =>
        t.allocationId === id ? { ...t, allocationId: null } : t
      )
    );
  };
  ...
  const handleSaveAllocation = async (data: Partial<BudgetAllocation>) => {
    if (allocationModal.editing) {
      ...
      if (res.data?.success)
        setAllocations((prev) =>
          prev.map((a) =>
            a.id === allocationModal.editing!.id ? res.data.data : a
          )
        );
    } else {
      const res = await allocationsAPI.create({
        ...data,
        targetType,
        targetId,
      });
  ...
  const handleSaveTask = async (data: Partial<Task>) => {
    if (taskModal.editing) {
      ...
      if (res.data?.success)
        setTasks((prev) =>
          prev.map((t) => (t.id === taskModal.editing!.id ? res.data.data : t))
        );
    } else {
    ...
    <div className="space-y-3">
      ...
      {allocations.map((alloc) => {
        ...
        return (
          <div
            key={alloc.id}
            className="rounded-2xl border border-slate-200/60 dark:border-slate-700/60 overflow-hidden"
          >
            <button
              onClick={() =>
                setExpanded((prev) => ({ ...prev, [alloc.id]: !isOpen }))
              }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[10px] text-slate-400 dark:text-slate-500">
                  {[formatAmount(alloc.amount, alloc.currency), alloc.period]
                    .filter(Boolean)
                    .join(" · ")}
              ...
              {isAdmin && (
                ...
                >
                  <button
                    onClick={() =>
                      setAllocationModal({ open: true, editing: alloc })
                    }
              ...
              <div className="p-3 space-y-2 bg-white/40 dark:bg-slate-900/30">
                {allocTasks.map((t) => (
                  <TaskCard
                    ...
                    onVote={isAdmin ? undefined : handleVote}
      ...
      {isAdmin && (
        <button
          onClick={() =>
            setTaskModal({ open: true, editing: null, allocationId: null })
          }
      ...
      {!isAdmin && allocations.length === 0 && standaloneTasks.length === 0 && (
        <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-3">
          Нет выделений по данному объекту
        </p>
      )}
