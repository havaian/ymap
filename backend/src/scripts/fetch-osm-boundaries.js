...
function toGeoJSON(payload, debug = false) {
    ...
    // Порядок фиксируется по osmId. Overpass не обещает стабильного порядка
    // элементов между запросами, и без сортировки повторное скачивание тех же
    // границ давало файл с другими байтами при том же содержимом. Дальше это
    // расходится: новый отпечаток файла - переимпорт границ бутстрапом и
    // повторное упрощение геометрии двухсот районов, всё впустую, плюс
    // четырёхмегабайтная строка в diff, по которой ничего не видно.
    features.sort((a, b) => (a.properties.osmId ?? 0) - (b.properties.osmId ?? 0));
