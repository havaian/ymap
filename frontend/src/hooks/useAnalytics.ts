...
export const useAnalytics = () => {
  ...
  const [budgetAnalytics, setBudgetAnalytics] =
  ...
  const fetchAll = useCallback(
    async (rc?: number | null, p?: number | null) => {
      try {
        setLoading(true);
        setError(null);
        ...
        const [overviewRes, scoringRes, regionsRes, issuesRes, cropsRes] =
          await Promise.all([
        ...
        try {
          ...
          const totalCommitted = withBudget.reduce(
            (s, d) => s + d.budgetCommittedUZS,
            0
          );
          const totalSpent = withBudget.reduce(
            (s, d) => s + d.budgetSpentUZS,
            0
          );
          const totalResolved = withBudget.reduce(
            (s, d) => s + d.resolvedCount,
            0
          );
          ...
          setBudgetAnalytics({
            totals: {
              ...
              executionRate:
                totalCommitted > 0
                  ? Math.round((totalSpent / totalCommitted) * 100)
                  : 0,
              costPerResolved:
                totalResolved > 0
                  ? Math.round(totalSpent / totalResolved)
                  : null,
            },
            byDistrict: withBudget
              ...
              .map((d) => ({
                ...
                costPerResolved:
                  d.resolvedCount > 0
                    ? Math.round(d.budgetSpentUZS / d.resolvedCount)
                    : null,
                budgetPerKm2:
                  d.areaKm2 > 0
                    ? Math.round(d.budgetCommittedUZS / d.areaKm2)
                    : 0,
              })),
          });
        }
...
export function useTrends(months = 12) {
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get(`/analytics/trends`, { params: { months } })
...
export function useResolution(regionCode?: number | null) {
  ...
  useEffect(() => {
    ...
    api
      .get("/analytics/resolution", { params })
      .then((res) => setData(res.data))
...
export function useEfficiency(regionCode?: number | null) {
  ...
  useEffect(() => {
    ...
    if (regionCode != null) params.regionCode = regionCode;
    api
      .get("/analytics/efficiency", { params })
      .then((res) => setData(res.data))
...
export function useDistrictProfile(name: string | null) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get(`/analytics/district/${encodeURIComponent(name)}`)
      .then((res) => setData(res.data))
