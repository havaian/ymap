...
import {
  RefreshCw, Database, Trash2, AlertCircle, CheckCircle2,
  Loader2, BarChart3, Cloud
} from 'lucide-react';
...
function SyncProgress({ job, onDone }: { job: JobState; onDone: () => void }) {
  ...
  return (
    <div className={`p-5 rounded-2xl border space-y-4 ${
      isError ? 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30'
      : isDone ? 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30'
      ...
      <div className="flex items-center gap-3">
        <div className={`flex-shrink-0 ${isError ? 'text-red-600 dark:text-red-400' : isDone ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'}`}>
          {isError ? <AlertCircle className="w-5 h-5" /> : isDone ? <CheckCircle2 className="w-5 h-5" /> : <Loader2 className="w-5 h-5 animate-spin" />}
        </div>
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-black ${isError ? 'text-red-800 dark:text-red-300' : isDone ? 'text-green-800 dark:text-green-300' : 'text-blue-800 dark:text-blue-300'}`}>
            {isError ? 'Ошибка синхронизации' : isDone ? 'Синхронизация завершена!' : 'Синхронизация...'}
          </p>
          {!isDone && !isError && (
      ...
      {!isDone && !isError && job.total > 0 && (
        <div className="space-y-1">
          <div className="h-2 bg-blue-200 dark:bg-blue-900/60 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 dark:bg-blue-400 rounded-full transition-all duration-200" style={{ width: `${phasePct}%` }} />
          </div>
          <p className="text-[10px] text-blue-600 dark:text-blue-400 font-bold text-right">{phasePct}%</p>
        </div>
      )}

      {isError && <p className="text-xs text-red-600 dark:text-red-400 font-medium">{job.error}</p>}

      {isDone && job.result && (
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
            <div className="text-lg font-black text-green-700 dark:text-green-400">{job.result.inserted ?? 0}</div>
            <div className="text-slate-500 dark:text-slate-400 mt-0.5">Добавлено</div>
          </div>
          <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
            <div className="text-lg font-black text-blue-700 dark:text-blue-400">{job.result.updated ?? 0}</div>
            <div className="text-slate-500 dark:text-slate-400 mt-0.5">Обновлено</div>
          </div>
          <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
...
export const AdminDataView: React.FC<AdminDataViewProps> = ({ onDataImported }) => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setSyncJob({ jobId, status: 'running', phase: 'fetching_ssv', progress: 0, total: 0, result: null, error: null });
      } else {
        setSyncJob({ jobId: '', status: 'error', phase: '', progress: 0, total: 0, result: null, error: response.data.message || 'Ошибка запуска' });
      }
    } catch (error: any) {
      setSyncJob({ jobId: '', status: 'error', phase: '', progress: 0, total: 0, result: null, error: error.response?.data?.message || error.message || 'Ошибка' });
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleSeedData = async () => {
    ...
    try {
      ...
      setSeedResult(response.data.success ? response.data.data : { error: response.data.message });
  ...
  const handleClearSeeded = async () => {
    ...
    try {
      ...
      setClearResult(response.data.success ? response.data.data : { error: response.data.message });
      if (response.data.success && onDataImported) onDataImported();
    } catch (error: any) {
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Синхронизация объектов и тестовые данные</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-white">Синхронизация объектов</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Загрузка актуальных данных из duasr.uz (ФАП, детские сады, школы)</p>
                </div>
              </div>
            </div>
                ...
                <>
                  <div className="flex items-start gap-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                      Синхронизация загружает все объекты из трёх источников (/api4/ssv, /api4/bogcha, /api4/maktab44)
                      и обновляет базу данных без удаления существующих данных. Координаты расставляются случайно в пределах района.
                    </p>
                  </div>
                  <button
                    onClick={handleSync}
                  ...
                  >
                    {isSyncing ? <><Loader2 className="w-5 h-5 animate-spin" /> Запуск...</> : <><RefreshCw className="w-5 h-5" /> Синхронизировать объекты</>}
                  </button>
                </>
              ) : (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-white">Генерация тестовых обращений</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Создать случайные обращения для демонстрации</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Кол-во обращений</label>
                  <input
                    type="number"
                    value={issuesCount}
                    onChange={(e) => setIssuesCount(parseInt(e.target.value) || 100)}
                    min={10}
                    max={5000}
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-3 cursor-pointer pb-3">
                    <div
                      ...
                      className={`w-12 h-6 rounded-full transition-colors ${includeComments ? 'bg-purple-600' : 'bg-slate-300 dark:bg-slate-600'} relative cursor-pointer`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all ${includeComments ? 'left-7' : 'left-1'}`} />
                    </div>
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Комментарии</span>
                  </label>
                </div>
              </div>
              ...
              >
                {isSeeding ? <><Loader2 className="w-5 h-5 animate-spin" /> Генерация...</> : <><Database className="w-5 h-5" /> Сгенерировать данные</>}
              </button>
              ...
              {seedResult && (
                <div className={`p-4 rounded-2xl border ${seedResult.error ? 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30' : 'bg-purple-50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-900/30'}`}>
                  {seedResult.error ? (
                    <p className="text-xs text-red-600 dark:text-red-400 font-medium">{seedResult.error}</p>
                  ) : (
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-purple-600 dark:text-purple-400">{seedResult.generated || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Обращений</div>
                      </div>
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-purple-600 dark:text-purple-400">{seedResult.users || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Пользователей</div>
                      </div>
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-purple-600 dark:text-purple-400">{seedResult.comments || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Комментариев</div>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-red-200 dark:border-red-900/30 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30 rounded-2xl">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-xs text-red-700 dark:text-red-400">
                    Удалит все автоматически созданные обращения, комментарии и тестовых пользователей.
                    Данные объектов затронуты не будут.
                  </p>
                </div>
              </div>
              ...
              >
                {isClearing ? <><Loader2 className="w-5 h-5 animate-spin" /> Очистка...</> : <><Trash2 className="w-5 h-5" /> Очистить тестовые данные</>}
              </button>
              ...
              {clearResult && (
                <div className={`p-4 rounded-2xl border ${clearResult.error ? 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30' : 'bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30'}`}>
                  {clearResult.error ? (
                    <p className="text-xs text-red-600 dark:text-red-400 font-medium">{clearResult.error}</p>
                  ) : (
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-green-700 dark:text-green-400">{clearResult.issues || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Обращений</div>
                      </div>
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-green-700 dark:text-green-400">{clearResult.comments || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Комментариев</div>
                      </div>
                      <div className="bg-white dark:bg-slate-900/50 p-2 rounded-xl text-center">
                        <div className="text-lg font-black text-green-700 dark:text-green-400">{clearResult.users || 0}</div>
                        <div className="text-slate-500 dark:text-slate-400 mt-0.5">Пользователей</div>
                      </div>
                    </div>
                  )}
