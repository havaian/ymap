...
export const BurgerMenu: React.FC<BurgerMenuProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-80 bg-white dark:bg-slate-900 z-[1002] shadow-2xl transform transition-transform duration-100 ease-out flex flex-col
      ...
      >
        <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 transition-colors duration-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              onClick={onClose}
              className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition text-slate-500 dark:text-slate-400"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white shadow-lg
              ${
                currentUser?.role === UserRole.ADMIN
                  ? "bg-indigo-600"
                  : "bg-blue-500"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5">
                ...
                {currentUser?.role === UserRole.ADMIN && (
                  <span className="bg-red-500 text-white text-[7px] font-black px-1 rounded uppercase tracking-tighter">
                    Admin
                  </span>
                )}
              </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          <p className="px-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 mt-4">
            Навигация
          </p>
          ...
          {isAdmin && (
            <>
              ...
              <p className="px-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">
                Админ-панель
              </p>
          ...
          <div className="mt-8">
            ...
            <button
              onClick={onOpenAbout}
              className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition font-bold text-sm"
            >
              <Info className="w-5 h-5" />О платформе
            </button>
          </div>
        </nav>
