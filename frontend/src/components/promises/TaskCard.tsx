...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  const hasVotedConfirm = userId
    ? task.votes.confirmed.includes(userId)
    : false;
    ...
    <div className="bg-white/80 dark:bg-slate-900/60 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 overflow-hidden">
      ...
      <div className="px-4 py-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          <span
            className={`inline-flex items-center gap-1 text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${cfg.color}`}
          >
            {cfg.icon}
            {cfg.label}
          </span>
